...
class PolicyGradientAgent(FNAgent):
    ...
    def scaler_path(self, model_path):
        frame, _ = os.path.splitext(model_path)
        frame += "_scaler.pkl"
        return frame
    ...
    def estimate(self, s):
        ...
        actions = self.model.predict(normalized)[0]
        return actions

    def update(self, states, actions, rewards):
        normalizers = self.scaler.transform(states)
        actions = np.array(actions)
        rewards = np.array(rewards)
        self._updater([normalizers, actions, rewards])
